...
def lab3Question4(number_1, number_2, number_3):
    ...
    if not (type(number_1) in [int, float] and type(number_2) in [int, float] and type(number_3) in [int, float]):
        return None
...
def lab3Question5(temperature, scale_used):
    ...
    if not (type(temperature) in [int, float]) or scale_used not in ["C", "F"]:
        return "Invalid"
    if scale_used == "C":
        if temperature <= 0:
            return "Solid"
